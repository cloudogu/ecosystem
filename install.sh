...
# The order matters twice and neither condition is enforced anywhere, so do not move this line:
#  - it has to run AFTER firewall.sh, because the post-install script of cesappd only creates its ufw rule for port
#    50051 while ufw is already active. Without that rule the admin dogu cannot reach cesappd.
#  - it has to run AFTER install-ces-packages.sh, because cesappd expects a matching restic version to be installed
#    (see the requirements in the cesappd README). Its post-install only warns about this, it does not fail.
# Consequently no package may declare a dependency on cesappd: apt would install it early as a side effect and this
# explicit install would silently become a no-op.
